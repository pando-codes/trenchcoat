...
**Plugin (Python):**
- Modify: `claude-plugin/lib/telemetry.py` — extend `parse_agent_transcript` to sum tokens + capture model
- Modify: `claude-plugin/hooks/stop.py` — parse main transcript, emit tokens/model
- Modify: `claude-plugin/hooks/subagent_stop.py` — pass tokens/model from parsed transcript
...
**Files:**
- Modify: `claude-plugin/lib/telemetry.py` (function at line ~377)

- [ ] **Step 1: Read the current function**

Open `claude-plugin/lib/telemetry.py`. Find `parse_agent_transcript` (around line 377). It currently returns `tool_counts`, `total_tools`, `turns`.
...
**Files:**
- Modify: `claude-plugin/hooks/stop.py`
- Modify: `claude-plugin/hooks/subagent_stop.py`

- [ ] **Step 1: Update `stop.py`**

Replace the entire contents of `claude-plugin/hooks/stop.py` with:
...
Replace the `write_event` call in `claude-plugin/hooks/subagent_stop.py` with:
...
```bash
git add claude-plugin/hooks/stop.py claude-plugin/hooks/subagent_stop.py
